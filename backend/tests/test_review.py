...
def test_get_reviews_by_food(auth_client, db_session, test_user):
    ...
    # 조회
    response = auth_client.get(f"/reviews/food/{food_id}")
    assert response.status_code == 200
...
def test_update_review(auth_client, db_session, test_user):
    ...
    # 리뷰 생성
    response = auth_client.post("/reviews/", json={
        "food_id": food_id,
        "content": "초기 리뷰",
        "rating": 3
    })
    ...
    # 리뷰 수정
    update_resp = auth_client.patch(f"/reviews/{review_id}", json={
        "content": "수정된 리뷰",
        "rating": 4
    })
...
def test_delete_review(auth_client, db_session, test_user):
    ...
    # 리뷰 삭제
    delete_resp = auth_client.delete(f"/reviews/{review_id}")
    assert delete_resp.status_code == 204

    # 삭제 확인 (내 리뷰 목록에서 조회 불가)
    get_resp = auth_client.get("/reviews/me")
