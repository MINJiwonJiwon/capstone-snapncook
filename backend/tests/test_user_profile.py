...
def test_update_profile_nickname_and_image(authenticated_headers):
    ...
    res = client.patch("/users/me", headers=authenticated_headers, json=data)
...
def test_change_password(authenticated_headers):
    ...
    res = client.patch("/users/me/password", headers=authenticated_headers, json=new_password_data)
    assert res.status_code == 200
    assert res.json()["message"] == "Password updated successfully"

def test_get_social_status(authenticated_headers):
    res = client.get("/users/me/social", headers=authenticated_headers)
    assert res.status_code == 200
...
def test_disconnect_social_account_fails_for_non_social_user(authenticated_headers):
    ...
    res = client.delete("/users/me/social/naver", headers=authenticated_headers)
    assert res.status_code == 400
    assert res.json()["detail"] == "No such social provider connected"

def test_delete_current_user(authenticated_headers):
    res = client.delete("/users/me", headers=authenticated_headers)
    assert res.status_code == 204

    # 삭제 이후 access 시도 시 401 또는 404
    res_check = client.get("/auth/me", headers=authenticated_headers)
